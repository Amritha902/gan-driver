...
%GAN_MASTER  Regenerate every result in the Review-I deck from the raw CSVs.
%
%  A FUNCTION FILE, not a script, and that is deliberate. MATLAB requires a
%  script's local functions to sit at the END of the file; Octave does not
%  hoist them, so at the end they are undefined when the body calls them.
%  The two rules cannot both be met by a plain script. In a function file
%  subfunctions are visible regardless of order, in BOTH tools.
%
%  Run it the same way as before -- type  gan_master  at the prompt.
