        ...
        for sh in _s.shapes:
            if sh.has_text_frame and u"What the data supports" in sh.text_frame.text:
                blocks = [
                    ...
                    # left rather than work already presented.
                    [(u"Review-III \u2014 place-and-route on a chosen board, then a "
                      u"hardware half-bridge measured on a bench. That is the "
                      u"whole of the remaining risk: nothing here has been "
                      u"built.", N)],
                    [(u"Also at Review-III \u2014 measure what a two-setting "
                      u"controller saves at the light-load point, where the best "
                      u"dead time differs.", N)],
                    [(u"Every number in this deck is regenerated by a named script, "
                      u"in ngspice or in Vivado — 66,924 transient simulations "
...
RENUM = [
    (u"Result 2 — the driver setting moves the converter",
     u"Result 2 — the driver setting moves the converter"),
    (u"Result 2 — re-tuning is worth only 3.5 %",
     u"Result 3 — re-tuning is worth only 3.5 %"),
