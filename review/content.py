...
SLIDE7B = [
    ([("Where we are", B)], 0),
    ([("Review-I is complete. ", B),
      ("The failure is reproduced and fixed, the 720-word search is finished at every corner, "
       "and the answer has been stress-tested against the device model, the board layout and "
       "an EMI objective.", False)], 1),
    ([("What we aim to do next", B)], 0),
    ([("Review-II \u2014 put it in silicon, not fabric.", B),
      (" A 200 MHz FPGA can only place a dead-time edge every 5 ns; the light-load corner wants "
       "15 ns and the rest want 5, so the grid is coarse exactly where the benefit lives. "
       "Drawing the segmented output stage at transistor level in SKY130 removes that "
       "quantisation, and lets us re-run the ceiling on real devices instead of ideal switches.",
       False)], 1),
    ([("Review-III \u2014 measure one corner in hardware.", B),
      (" Build the half-bridge and measure the crosstalk margin against the simulated 1.65 V. "
       "One measured corner is what turns this from a simulation study into a result, and until "
       "that exists the title says simulation.", False)], 1),
    ([("The risk we already know about.", B),
      (" A 1.8 V / 3.3 V teaching PDK cannot take a 5 V gate rail, so securing a 5 V-capable "
       "PDK is the first milestone and the one that can stall the rest.", False)], 1),
    ([("Tools, and how to check any of it", B)], 0),
    ([("ngspice 42 \u00b7 LTspice 24 \u00b7 Icarus Verilog \u00b7 Xilinx Vivado 2024.1.2 \u00b7 "
       "Cadence Spectre \u00b7 SKY130 \u00b7 Python / NumPy \u00b7 MATLAB Online and Octave. "
       "Every number is regenerated by a named script in ", False),
